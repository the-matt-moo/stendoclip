...
Win32 requires clipboard listener and hotkey registration on the same thread as the message pump. Running two separate message loops (one for clipboard, one for hotkeys) would require cross-thread synchronization and duplicated pump logic. A single hidden top-level window receiving both message types is simpler and matches how native Windows apps work. Unlike `HWND_MESSAGE`, a top-level owner receives the taskbar-recreation broadcast and can own a correctly dismissing tray menu. Goroutines handle the async work (fsnotify, paste delays) without blocking the pump.
